...
import '../models/location/spot.dart';
...
/// session management, spot boundary detection, and paginated history queries.
///
...
class LocationService {
  ...
  ///
  /// Submits a single real-time GPS ping. Automatically detects spot
  ...
  /// Spot boundary detection is performed per ping.
  ///
  /// Throws [ApiError] on any non-2xx response.
  ...
  Future<LocationSession> startSession(StartSessionRequest request) async {
    try {
      ...
      return LocationSession.fromJson(
  ...
  /// GET /mobile/v1/location/spots
  ///
  /// Returns all active spot/checkpoint zones for the company.
  ///
  /// Throws [ApiError] on any non-2xx response.
  Future<List<Spot>> listSpots() async {
    try {
      final response = await _backendDio.get<Map<String, dynamic>>(
        '/mobile/v1/location/spots',
      );
      final list = response.data!['spots'] as List<dynamic>;
